...
public:
    ...
    /// Get a pointer to a resized memory block
    /// @param[in]  pBlock - block of memory to resize
    /// @param[in]  oldSize - old size of the memory block
    /// @param[in]  newSize - new size of the memory block
    /// @return     Return pointer to a new block that is resized. Othewise NULL if unsuccessful
    void *reallocate(void *pBlock, size_t oldSize, size_t newSize);
